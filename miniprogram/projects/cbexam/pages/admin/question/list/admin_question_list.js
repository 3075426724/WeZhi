...
Page({
	...
	_del: async function (e) {
		...
		let callback = async () => {
			try {
				...
				await cloudHelper.callCloudSumbit('admin/question_del', params, opts).then(res => {
	...
	_setStatus: async function (e) {
		...
		try {
			await cloudHelper.callCloudSumbit('admin/question_status', params).then(res => {
				pageHelper.modifyListNode(id, this.data.dataList.list, 'QUESTION_STATUS', status, '_id');
				this.setData({
					dataList: this.data.dataList
				});
	...
	_getSearchMenu: function () {
		...
		let sortMenus = [
			...
			{ label: '单选题', type: 'type', value: '单选题' },
			{ label: '多选题', type: 'type', value: '多选题' }
		]
