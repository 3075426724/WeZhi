...
Page({
	...
	_loadDetail: async function (isDel) {
		...
		let data = await cloudHelper.callCloudData('admin/rank_data_get', params, options);
	...
	bindOpenTap: function (e) {
		fileHelper.openDoc('排行榜数据', this.data.url);
	},
	...
	bindExportTap: async function (e) {
		try {
			...
			await cloudHelper.callCloudData('admin/rank_data_export', params, options).then(res => {
	...
	bindDelTap: async function (e) {
		try {
			...
			await cloudHelper.callCloudData('admin/rank_data_del', {}, options).then(res => {
				this.setData({
					url: '',
					time: ''
				});
