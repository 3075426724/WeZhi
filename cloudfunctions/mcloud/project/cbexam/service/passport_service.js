...
class PassportService extends BaseProjectService {
	...
	async getMyDetail(userId) {
		...
		let fields = 'USER_MOBILE,USER_NAME,USER_FORMS,USER_OBJ,USER_STATUS,USER_ANSWER_CNT,USER_ANSWER_SCORE,USER_CHECK_REASON'
		let user = await UserModel.getOne(where, fields);

		if (!user) return user;

		let cnt = await AnswerModel.count({ ANSWER_USER_ID: userId, ANSWER_TYPE: 1 });
		user.USER_TODAY_CNT = cnt;
