...
class AdminUserController extends BaseProjectAdminController {
	...
	async rankDataGet() {
		...
		if (input.isDel === 1)
			await service.deleteRankDataExcel(); //先删除 

		return await service.getRankDataURL();
	}

	/** 导出数据 */
	async rankDataExport() {
		await this.isAdmin();
		...
		let service = new AdminUserService();
		return await service.exportRankDataExcel();
	}
	...
	async rankDataDel() {
		...
		let service = new AdminUserService();
		return await service.deleteRankDataExcel();
	}
}
