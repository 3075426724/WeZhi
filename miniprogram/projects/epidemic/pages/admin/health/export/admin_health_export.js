...
Page({
	...
	onLoad: function (options) {
		...
		HealthBiz.setHealthTitle(this, this.data.cateId, '', '-填报数据导出');
	...
	_loadDetail: async function (isDel) {
		...
		let data = await cloudHelper.callCloudData('admin/health_data_get', params, options);
	...
	bindOpenTap: function (e) {
		fileHelper.openDoc('填报数据', this.data.url);
	},
	...
	bindExportTap: async function (e) {
		try {
			...
			let params = {
				...
				fields: projectSetting['HEALTH_FIELDS' + this.data.cateId]
			}

			await cloudHelper.callCloudData('admin/health_data_export', params, options).then(res => {
	...
	bindDelTap: async function (e) {
		try {
			...
			await cloudHelper.callCloudData('admin/health_data_del', {}, options).then(res => {
				this.setData({
					url: '',
					time: ''
				});
